...
TEST(Nurbs, UniformConstructionInitializesHomogeneousWeights) {
  ...
  const auto weights = nurbs.weights();
  EXPECT_TRUE(torch::equal(
      weights, torch::ones({3}, weights.options())));
...
TEST(Nurbs, UniformConstructionSupportsExplicitHomogeneousCoefficients) {
  ...
  EXPECT_TRUE(torch::equal(nurbs.coeffs(0).cpu(), coeffs[0]));
  EXPECT_TRUE(torch::equal(nurbs.coeffs(1).cpu(), coeffs[1]));
  EXPECT_TRUE(torch::equal(nurbs.weights().cpu(), coeffs[2]));

  coeffs[2].fill_(9.0);
  EXPECT_FALSE(torch::equal(nurbs.weights().cpu(), coeffs[2]));
}
